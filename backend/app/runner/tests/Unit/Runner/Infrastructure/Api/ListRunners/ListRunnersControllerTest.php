...
namespace App\Tests\Unit\Runner\Runner\Infrastructure\Api\ListRunners;
...
use App\Runner\Runner\Infrastructure\Api\ListRunners\ListRunnersController;
...
#[CoversClass(ListRunnersController::class)]
final class ListRunnersControllerTest extends TestCase
{
    ...
    public function defaults_to_a_limit_of_20_when_none_is_requested(): void
    {
        ...
        $this->bus
            ...
            ->willReturnCallback(static function (object $message): Envelope {
                Assert::assertInstanceOf(ListRunnersQuery::class, $message);
        ...
        $controller = new ListRunnersController($this->bus, $this->organizationContext);
    ...
    #[Test]
    public function lists_live_runners_by_default_and_archived_ones_on_request(): void
    {
        // Arrange
        $this->stubOrganization();
        ...
        $this->bus
            ->method('dispatch')
            ->willReturnCallback(static function (object $message) use (&$requestedFlags): Envelope {
                Assert::assertInstanceOf(ListRunnersQuery::class, $message);
                $requestedFlags[] = $message->archived;
        ...
        $controller($this->user, new Request(query: ['archived' => 'true']));

        // Assert
        Assert::assertSame([false, true], $requestedFlags);
    }
    ...
    public function maps_the_returned_page_into_the_response_payload(): void
    {
        ...
        $runner = new RunnerOverview(
            id: 'runner-1',
            name: 'fleet-runner-01',
            status: 'idle',
            lastSeenAt: '2026-08-01T10:00:00+00:00',
            createdAt: '2026-07-01T00:00:00+00:00',
            archivedAt: '2026-08-05T09:00:00+00:00',
        );
        ...
        Assert::assertCount(1, $payload['runners']);
        Assert::assertSame('runner-1', $payload['runners'][0]['id']);
        Assert::assertSame('fleet-runner-01', $payload['runners'][0]['name']);
        Assert::assertSame('idle', $payload['runners'][0]['status']);
        Assert::assertSame('2026-08-05T09:00:00+00:00', $payload['runners'][0]['archivedAt']);
        Assert::assertSame(21, $payload['pagination']['total']);
        Assert::assertTrue($payload['pagination']['hasNextPage']);
        Assert::assertFalse($payload['pagination']['hasPreviousPage']);
    }
