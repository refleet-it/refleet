...
final class StaticAssertionsToPHPUnitAssertStaticMethodRector extends AbstractRector
{
    /**
     * @var string
     */
    ...
    public function refactor(Node $node): ?Node
    {
        ...
        $this->traverseNodesWithCallable($node, function (Node $node) use (&$hasChanged): ?StaticCall {
            ...
            if (!$this->isObjectType($node->class, new ObjectType(PHPUnit\Framework\TestCase::class))) {
                return null;
            }
    ...
    {
        return new \Symplify\RuleDocGenerator\ValueObject\RuleDefinition('convert static call assertions in classes extended TestCase to static call from PHPUnit Assert object', []);
    }
}
